...
import json, pprint, os, shutil, subprocess
from datetime import date, timedelta


sar_dir_path = '/var/log/sa'

def get_system_stats(dt, stat_type):
  ...
  try:
    ...
    if d:
      if 'sysstat' not in d or 'hosts' not in d['sysstat'] :
        raise Exception("Error getting system statistics : No statistics available for any host.")
      d1, err = process_host(d['sysstat']['hosts'][0], stat_type)
      if err:
        raise Exception(err)
      if not d1:
        raise Exception('Error getting statistics for host %s : %s'%(d['sysstat']['hosts'][0]['nodename'], err))
  except Exception, e:
  ...
  try:
    ...
    for stat in id['statistics']:
      ...
      if stat_type == 'cpu':
        ...
        d['iowait'].append([stat['timestamp']['time'], stat['cpu-load'][0]['iowait']])
        d['system'].append([stat['timestamp']['time'], stat['cpu-load'][0]['system']])
        d['idle'].append([stat['timestamp']['time'], stat['cpu-load'][0]['idle']])
        d['user'].append([stat['timestamp']['time'], stat['cpu-load'][0]['user']])
      elif stat_type == 'memory':
        if 'memory' not in stat:
          continue
        d['memused-percent'].append([stat['timestamp']['time'], stat['memory']['memused-percent']])
        d['memused'].append([stat['timestamp']['time'], stat['memory']['memused']])
        d['memfree'].append([stat['timestamp']['time'], stat['memory']['memfree']])
      elif stat_type == 'queue':
        if 'queue' not in stat:
          continue
        d['ldavg-1'].append([stat['timestamp']['time'], stat['queue']['ldavg-1']])
        d['ldavg-5'].append([stat['timestamp']['time'], stat['queue']['ldavg-5']])
        d['ldavg-15'].append([stat['timestamp']['time'], stat['queue']['ldavg-15']])
        d['runq-sz'].append([stat['timestamp']['time'], stat['queue']['runq-sz']])
      elif stat_type == 'disk':
        if 'disk' not in stat:
          continue
        d = {}
        ...
        if 'network' in stat and 'net-dev' in stat['network']:
          for iface in stat['network']['net-dev']:
            ...
            d[iface['iface']]['ifutil-percent'].append([stat['timestamp']['time'], iface['ifutil-percent']])
  except Exception, e:
...
def load_stats(dt):
  ...
  try :
    if os.path.exists('/var/log/sa/sa%s'%dt):
      proc = subprocess.Popen(['sadf', '-T', '-j', '/var/log/sa/sa%s'%dt, '--', '-qurd', '-n', 'DEV'], stdout = subprocess.PIPE)
