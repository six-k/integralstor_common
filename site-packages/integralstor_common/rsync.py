...
def create_rsync_share(name,path,comment,list,readonly,uid,gid):
  try:
    ...
    check,err = db.read_single_row(db_path, "select * from rsync_shares where name='%s'"%name)
    if  check:
      raise Exception("Share already exists.Use a different share name") 

    cmd_list = []
    cmd = ["insert into rsync_shares (name,path,comment,list,readonly,uid,gid) values(?,?,?,?,?,?,?)", (name,path,comment,list,readonly,uid,gid)]
    cmd_list.append(cmd)
...
def get_rsync_share_details(name):
  ...
  try:
    ...
    if err:
      raise Exception(err)
    share,err = db.read_single_row(db_path, "select * from rsync_shares where name='%s'"%name)
    if not share:
...
def load_shares_list():
  ...
  try:
    ...
    if err:
      raise Exception(err)
    l, err = db.read_multiple_rows(db_path, 'select * from rsync')
    if err:
      raise Exception(err)
  except Exception, e:
