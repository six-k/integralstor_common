from integralstor_common import common,db,command, audit
import crontab, cron_descriptor
import re,time,datetime,socket, getpass, os
...
#def add_cron_entry(command, description, min="1",hour='*',day='*',dow='*',month='*',user='root'):
def add_cron_task(command, description, min="1",hour='*',day='*',dow='*',month='*',user='root'):
    cron_task_id = None
    try:
        ...
        cmd = ['insert into cron_tasks(command,description) values (?,?)', (command,description)]
        #print cmd
...
#def get_all_cron_tasks(user='root'):
    ...
    try:
        ...
        if cron_db_entries:
            ...
            for cron_db_entry in cron_db_entries:
                ...
                if jobs:
                    for job in jobs:
                        cron_dict['schedule_description'] = job.description(use_24hour_time_format=True)
                        cron_dict['job'] = job
                        break
...
def remove_cron(cron_task_id, user='root'):
    try:
        db_path, err = common.get_db_path()
        if err:
            raise Exception(err) 
        ...
        if err:
            raise Exception(err) 
        cmd_list = []
        cmd_list.append(['delete from cron_tasks where cron_task_id=%d'%cron_task_id])
        cmd_list.append(['update tasks set status="cancelled" where cron_task_id=%s and status is not "completed"'%cron_task_id])
        if tasks:
            for task in tasks:
                cmd_list.append(['update subtasks set status="cancelled" where task_id=%d  and status is not "completed"'%task['task_id']])
        #print cmd_list
...
6. attempts : The total number of attempts in case if the command fails. Defaults to 3 attempts
If attempts value is -2, keep appempting forever.
"""

def add_task(description,subtask_list,task_type_id = 0, cron_task_id = 0, node=socket.getfqdn(),initiate_time=None,attempts=3,run_as_user_name='root', retry_interval=1):
    '''
    TODO 
    ...
    try:
        ...
        cmd = "insert into tasks (description,task_type_id, node, run_as_user_name, attempts, cron_task_id, retry_interval, create_time, initiate_time) VALUES ('%s','%d','%s','%s','%d','%d','%d', '%d', '%d');"%(description, task_type_id, node, run_as_user_name, attempts, int(cron_task_id), retry_interval, now,initiate_time);
        #print cmd
        db_path, err = common.get_db_path()
        if err:
            raise Exception(err) 
        ...
        if row_id:
            ...
            for subtask in subtask_list:
                for description,command in subtask.iteritems():
                    ...
                    cmd =  "insert into subtasks (description,command,task_id) values ('%s','%s','%d');"%(description,command,row_id)
                    status,err = db.execute_iud(db_path,[[cmd],],get_rowid=True)
                    if err:
                        raise Exception('Error creating scheduled command : %s'%err)
        else:
...
def remove_task(task_id):
    try:
        ...
        if err:
            raise Exception(err)
        cmd_list = [['delete from tasks where task_id = %s'%task_id], ['delete from subtasks where task_id = %s'%task_id]]
        #print cmd_list
...
def get_tasks_by_cron_task_id(cron_task_id):
    ...
    try:
        ...
        if err:
            raise Exception(err) 
        query = 'select * from tasks where cron_task_id="%d"'%int(cron_task_id)
        #print query
...
#def get_scheduled_task(db_path,task_id):
...
"""
This function is to be called from cron. This will read the scheduler database, and execute all the required queries  do the attempts and logs errors and outputs.
"""
...
def process_tasks(node=socket.getfqdn()):
    try:
        ...
        tasks_query = "select * from tasks where node == '"+node+"' and (status == 'error-retrying' or status == 'queued') and (initiate_time <= '%d');" %(now)
        print tasks_query
        ...
        if tasks_to_process is not None:
            ...
            for task in tasks_to_process:
                ...
                cmd = "update tasks set status = 'running', last_run_time=%d where task_id = '%d'"%(now, task['task_id'])
                status,err = db.execute_iud(db_path,[[cmd],],get_rowid=True)
                #print status, err
                if err:
                    raise Exception(err)
                ...
                subtasks_query = "select * from subtasks where task_id == '%d' and (status == 'error-retrying' or status == 'queued') order by subtask_id"%task['task_id']
                ...
                for subtask in subtasks:
                    ...
                    status,err = db.execute_iud(db_path,[[status_update],],get_rowid=True)
                    if err:
                    ...
                    (out,return_code),err = command.execute_with_rc(subtask["command"],shell=True, run_as_user_name=run_as_user_name)
                    ...
                    if return_code == 0:
                        ...
                        status_update = "update subtasks set status = 'completed', return_code='%d' where subtask_id = '%d' and status is not 'cancelled';"%(return_code,subtask_id)
                        status,err = db.execute_iud(db_path,[[status_update],],get_rowid=True)
                        if err:
                            task_completed = False
                            break
                        ...
                        if attempts > 1 or attempts == -2:
                            status_update = 'update subtasks set status = "error-retrying", return_code="%d" where subtask_id = "%d" and status is not "cancelled";'%(return_code,subtask_id)
                        elif attempts in [0, 1]:
                            status_update = 'update scheduler_commands set status = "failed", return_code="%d"" where subtask_id = "%d" and status is not "cancelled";'%(return_code,subtask_id)
                        execute,err = db.execute_iud(db_path,[[status_update],],get_rowid=True)
                        task_completed = False
                        break

                if task_completed:
                    status_update = "update tasks set status = 'completed' where task_id = '%d'"%task['task_id']
                else:
                    if attempts > 1:
                        status_update = "update tasks set status = 'error-retrying', attempts = %d where task_id = '%d' and status is not 'cancelled'"%(attempts-1, task['task_id'])
                    elif attempts == -2:
                        status_update = "update tasks set status = 'error-retrying', attempts = %d where task_id = '%d' and status is not 'cancelled'"%(-2, task['task_id'])
                    else:
                        status_update = "update tasks set status = 'failed', attempts = '%d' where task_id = '%d' and status is not 'cancelled'"%(0, task['task_id'])
                status,err = db.execute_iud(db_path,[[status_update],],get_rowid=True)
                if err:
                    raise Exception(err)

                if task_completed:
                    audit.audit("task_complete", audit_str, None, system_initiated = True)
                else:
