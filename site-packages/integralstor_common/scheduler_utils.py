...
def remove_cron(cron_task_id, user='root'):
  try:
    ...
    cmd_list.append(['update tasks set status="cancelled" where cron_task_id=%s'%cron_task_id])
    if tasks:
      for task in tasks:
        cmd_list.append(['update subtasks set status="cancelled" where task_id=%d'%task['task_id']])
    #print cmd_list
