...
4. execute_time : The time at which the job has to be executed. Defaults to now timestamp
5. retries : The total number of retries in case if the command fails. Defaults to 3 retries
"""

def schedule_a_job(db_path,task_name,cmd_list,node=socket.getfqdn(),execute_time=None,retries=3):
  if not task_name or not cmd_list:
    ...
    try:
      if not execute_time:
        ...
        now = int(time.time())
      cmd =  "insert into scheduler_tasks (task_name,create_time,initiate_time,status,node) VALUES ('%s','%d','%d','%s','%s');"%(task_name,now,execute_time,"scheduled",node);
      row_id,err = execute_iud(db_path,[[cmd],],get_rowid=True)
      if row_id and not err:
        error = False
        for cmd in cmd_list:
          for name,cmd in cmd.iteritems():
            cmd =  "insert into scheduler_commands (name,command,task_ref_id,retries) values ('%s','%s','%d','%d');"%(name,cmd,row_id,retries)
...
def get_background_jobs(db_path,minutes=1440,node=None):
  ...
  try:
    ...
    if not tasks:
      return "No background jobs scheduled",False
    else:
      return tasks,False
  except Exception,e:
    return None,e
...
def get_background_job(db_path,task_id):
  tasks = "select task_name from scheduler_tasks where task_id=='%d'"%task_id
  try:
    tasks,err = read_multiple_rows(db_path,tasks)
    if not tasks:
      return "No jobs available",False
    else:
      return tasks,False
  except Exception,e:
    return None,e
...
def get_task_details(db_path,task_id):
  ...
  try:
    tasks,err = read_multiple_rows(db_path,tasks)[0]
    if not tasks:
      return None,False
    else:
      return tasks,False
...
def main():
  ...
  #  else:
  #    status,err = schedule_a_job(db_path,jobname,[{'cmd1':'echo123'},{'cmd2':'ecdsfsdho'},{'cmd3':'spsdfawd'}])
  #print execute_scheduler(db_path)
