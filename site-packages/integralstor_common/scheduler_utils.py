...
import re,time,datetime,socket, getpass
...
def create_cron(cron_name,min="1",hour='*',day='*',dow='*',month='*',command=None,log_file=None, user='root'):
  try:
...
def get_all_cron_jobs(user='root'):
  cron_list = []
  try:
    cron = crontab.CronTab(user)
    for job in cron:
...
def delete_cron_with_comment(name, user='root'):
  try:
    cron = crontab.CronTab(user)
...
def schedule_a_job(db_path,task_name,cmd_list,node=socket.getfqdn(),execute_time=None,retries=3,extra = {}, user_name='root'):
  deleteble = 0
  execute_after = -1
  if not task_name or not cmd_list:
    ...
    try:
      ...
      cmd =  "insert into scheduler_tasks (task_name,create_time,initiate_time,status,node,deleteble,execute_after, user_name) VALUES ('%s','%d','%d','%s','%s','%d','%d', '%s');"%(task_name,now,execute_time,"scheduled",node,deleteble,execute_after, user_name);
      print cmd
...
def execute_scheduler(db_path,node=socket.getfqdn()):
  try:
    ...
    tasks, err = db.read_multiple_rows(db_path,tasks)
    if err:
      raise Exception(err)
    for task in tasks:
      ...
      if not task['execute_after'] == -1:
        ...
        status,err = db.read_single_row(db_path,check)
      ...
      for query in db.read_multiple_rows(db_path,commands)[0]:
        ...
        status,err = db.execute_iud(db_path,[[status_update],],get_rowid=True)
        #print query['command']
        #print 'username is ', task['user_name']
        if task['user_name'] != current_user:
          #print 'not current user'
          #This task is not to be meant to be executed by the current user so switch to that user
          (out,ret),err = command.execute_with_rc(query["command"],True, run_as_user_name=task['user_name'])
          #print 'out ret err are ', out, ret, err
        else:
          (out,ret),err = command.execute_with_rc(query["command"],True)
        return_code = ret
        if out[0]:
...
def delete_task(task_id):
  ...
  execute,err_com = db.execute_iud(db_path,[[commands],],get_rowid=True)
  if not err_com:
    tasks = "delete from scheduler_tasks where task_id=='%d'"%task_id
    execute,err_task = db.execute_iud(db_path,[[tasks],],get_rowid=True)
