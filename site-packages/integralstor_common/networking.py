  ...
  try:
    ...
    if init_type not in ['systemd', 'init']:
      ...
      if err:
        ...
        for slave in slaves:
          cmd_slave = "nmcli con add type bond-slave con-name %s-%s ifname %s master %s" %(bond_name, slave, slave, bond_name)
          r, err = command.get_command_output (cmd_slave)
          if err:
            raise Exception (err)
...
def set_interface_ip_info(if_name, d):
  try:
    ...
    if init_type not in ['systemd', 'init']:
      ...
      cmd_set_ip = 'nmcli con mod %s ipv4.method manual connection.autoconnect yes ipv4.addresses %s ipv4.gateway %s ipv4.routes "%s %s"' %(if_name, if_addr, if_gw, if_nm_route, if_gw)
