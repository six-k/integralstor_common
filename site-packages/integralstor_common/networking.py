...
import time
from os import path
from integralstor_common import command, services_management, common
...
"""
Ensure that the passed mask is valid
"""
...
"""
Convert a dotted IP subnet into a CIDR format
"""
    ...
    try:
        ...
        if init_type not in ['systemd', 'init']:
            ...
            if os.path.isfile('/etc/modprobe.d/bonding.conf'):
                shutil.copyfile('/etc/modprobe.d/bonding.conf', '/etc/modprobe.d/BAK.bonding.conf')
            ...
            for slave in bid['by_master'][bond_name]['slaves']:
                if os.path.isfile('/etc/sysconfig/network-scripts/ifcfg-%s'%slave):
                    shutil.copyfile('/etc/sysconfig/network-scripts/ifcfg-%s'%slave, '/etc/sysconfig/network-scripts/BAK.ifcfg-%s'%slave)
                with open('/etc/sysconfig/network-scripts/ifcfg-%s'%slave, 'w') as f:
                    f.write('DEVICE=%s\n'%slave)
                    f.write('USERCTL=no\n')
                    f.write('ONBOOT=yes\n')
            ...
            if os.path.isfile('/etc/sysconfig/network-scripts/ifcfg-%s'%bond_name):
                shutil.copyfile('/etc/sysconfig/network-scripts/ifcfg-%s'%bond_name, '/etc/sysconfig/network-scripts/BAK.ifcfg-%s'%bond_name)
            if os.path.isfile('/etc/sysconfig/network-scripts/ifcfg-%s'%bond_name):
            ...
            try:
                ...
                os.remove('/etc/sysconfig/network-scripts/BAK.ifcfg-%s'%bond_name)
                filelist=glob.glob('/etc/sysconfig/network-scripts/BAK.ifcfg-*')
...
def create_bond(bond_name, slaves, mode, bootproto = None, addr_info = None):
    ...
    try:
        ...
        if init_type not in ['systemd', 'init']:
            ...
            cmd_bond = "nmcli con add type bond con-name %s ifname %s mode %d" %(bond_name, bond_name, mode)
            r, err = command.get_command_output (cmd_bond)
            if err:
                raise Exception (err)
            else:
                for slave in slaves:
                    # regardless of returned error on executing cmd_del, cmd_down
                    ...
                    cmd_slave = "nmcli con add autoconnect yes type bond-slave con-name %s-%s ifname %s master %s" %(bond_name, slave, slave, bond_name)
                    r, err = command.get_command_output (cmd_slave)
                    if err:
                        raise Exception (err)
            ...
            with open('/etc/sysconfig/network-scripts/ifcfg-%s'%bond_name, 'w') as f:
                ...
                f.write('ONBOOT=yes\n')
                f.write('USERCTL=no\n')
                f.write('BONDING_OPTS="mode=%d miimon=100 downdelay=200 updelay=200"\n'%mode)
            modified_file_list.append('/etc/sysconfig/network-scripts/ifcfg-%s'%bond_name)
            os.system('echo +%s > /sys/class/net/bonding_masters'%bond_name)
            ret, err = restart_networking()
            if not ret:
...
"""
Set the NIC bonding mode
"""
def set_bonding_type(bondname, bonding_type):
    try:
        if bonding_type not in [4, 6]:
            raise Exception( "Unsupported bonding type : %d"%bonding_type)
        with open('/etc/modprobe.d/bonding.conf', 'w') as f:
            f.write('#Generated by the IntegralStor script\n')
            f.write('alias %s bonding\n'%bondname)
        ...
        #print e
        return False, 'Error setting bonding type : %s'%str(e)
    else:
        return True, None
        ...
        for ifname, id in if_dict.items():
            if id['vlan']:
                ...
                if vlan_comps[0] not in if_dict.keys():
                    raise Exception('Incorrectly configured VLAN : %s. Base NIC missing.'%ifname)
...
def set_interface_ip_info(if_name, d):
    try:
        ...
        if init_type == 'systemd' and is_bond is True:
            ...
            cmd_set_ip = 'nmcli con mod %s ipv4.method manual connection.autoconnect yes connection.autoconnect-priority 99 connection.autoconnect-slaves 1 ipv4.never-default no ipv4.addresses %s ipv4.gateway %s ipv4.routes "%s %s"' %(if_name, if_addr, if_gw, if_nm_route, if_gw)
            ...
            if err:
                cmd_con_create = 'cmli con add type ethernet con-name %s ifname %s' %(if_name, if_name)
            ...
            cmd_con_down = "nmcli con down %s" %if_name
            r, err = command.get_command_output (cmd_con_down)
            if err and assert_con is not None:
                raise Exception (err)

            cmd_set_ip = 'nmcli con mod %s ipv4.method manual connection.autoconnect yes ipv4.never-default no ipv4.addresses %s ipv4.gateway %s ipv4.routes "%s %s"' %(if_name, if_addr, if_gw, if_nm_route, if_gw)
            r, err = command.get_command_output (cmd_set_ip)
            if err:
                raise Exception (err)
...
def create_vlan(if_name, vlan_id):
    try:
        ...
        file_name = '/etc/sysconfig/network-scripts/ifcfg-%s.%d'%(if_name, vlan_id)
...
def set_hosts_file_entry(hostname, ip):
...
def restart_networking():
    ...
    try:
        ret, err = services_management.change_service_status('network','restart')
        if err:
            raise Exception(err)
    except Exception, e:
...
# Pass a request.META, to this function, and enable X_Http_Forward in nginx,and this function will return the actual client ip of the client and not the gateway. Works over firewall
...
if __name__ == "__main__":
    ...
    set_hostname('check-330', 'check-dom')
...
def set_admin_servers(admin_server_list):
  """ Add the supplied IP list into the hosts file"""
  try:
    if not admin_server_list or len(admin_server_list) < 3 :
      raise Exception('Invalid admin server list supplied.')

    lines = None
    new_lines = []
    localhost_done = False
    with open('/etc/hosts', 'r') as f:
      lines = f.readlines()
    if lines:
      updated_list = []
      for line in lines:
        ...
        new_line = line
        #Scroll through all admin servers to see if this line needs tweaking
        found_ip = False
        for admin_server_tuple in list(enumerate(admin_server_list, start=1)):
          if components and components[0] == admin_server_tuple[1]:
            found_ip = True
            updated_list.append(admin_server_tuple[1])
            if 'admin_gridcell' not in line:
              #Currently not an admin gridcell so just add
              new_line = '%s admin_gridcell_%d.integralstor.lan\n'%(line.strip(), admin_server_tuple[0])
              break
            elif 'admin_gridcell_%d'%admin_server_tuple[0] not in line:
              #It is currently some other admin server so remove that and and then add.
              new_line = '%s '%components[0]
              for comp in components[1:]:
                if 'admin_gridcell' not in comp:
                  new_line = '%s %s'%(new_line, comp)
              new_line += ' admin_gridcell_%d.integralstor.lan\n'%admin_server_tuple[0]
            else:
              #It is already there and correct so leave it
              pass
            break
        #print line
        #print found_ip
        #print ('admin_gridcell' in line)
        if (not found_ip) and 'admin_gridcell' in line:
          #print 'Changing!'
          #IP is not one of our admin servers so we need to remove the admin server component if it has one.
          new_line = '%s '%components[0]
          for comp in components[1:]:
            if 'admin_gridcell' not in comp:
              new_line = '%s %s'%(new_line, comp)
        ...
        new_lines.append(new_line)
      for admin_server_tuple in list(enumerate(admin_server_list, start=1)):
        if admin_server_tuple[1] not in updated_list:
          new_lines.append('%s admin_gridcell_%d.integralstor.lan'%(admin_server_tuple[1], admin_server_tuple[0]))
    with open('/tmp/integralstor_newhosts', 'w') as f1:
      for line in new_lines:
        #print 'writing ', line
        f1.write('%s'%line)
    ...
    return False, 'Error setting admin servers : %s'%str(e)
  else:
    return True, None
'''
