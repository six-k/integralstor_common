...
def get_subnet_in_cidr_format(ip, subnet):
    # Given an IP and a subnet (as in 255.255..) string, returns the subnet
    # info in CIDR format (as in 192.168.1.0/24)
...
def set_interface_ip_info(if_name, d):
    try:
        ...
        if init_type == 'systemd' and is_bond is True:
            ...
            if err:
                cmd_con_create = 'cmli con add type ethernet con-name %s ifname %s' % (
                    if_name, if_name)
