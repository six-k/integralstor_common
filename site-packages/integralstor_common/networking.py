...
def set_interface_ip_info(if_name, d):
  try:
    ...
    if init_type not in ['systemd', 'init']:
    ...
    elif (init_type == 'systemd') and (if_name.find ("bond") is not -1):
      ...
      cmd_set_ip = 'nmcli con mod %s ipv4.method manual connection.autoconnect yes connection.autoconnect-priority 99 connection.autoconnect-slaves yes ipv4.never-default no ipv4.addresses %s ipv4.gateway %s ipv4.routes "%s %s"' %(if_name, if_addr, if_gw, if_nm_route, if_gw)
