...
"""
Get all disks by their by-id ids.
"""
...
"""
Get all disks by their scsi paths
"""
...
"""
Get all disks by their by-uuid ids.
"""
...
"""
Given a disk by name, check if its rotational or flash.
"""
...
"""
Given a disk by name, get its capacity.
"""
...
"""
Given a disk by name, get its partitions.
"""
def get_partitions(disk_name):
  ...
  try:
    if not disk_name:
      raise Exception("No disk name specified so cannot retrieve partitions")
    supplied_path = '/dev/%s'%disk_name
    id_list = []
    for (dirpath, dirnames, filenames) in os.walk('/dev/disk/by-id'):
      id_list = filenames
...
def get_mdraid_info(device):
  raid_list = []
  try:
    lines, err = command.get_command_output('mdadm --detail /dev/%s'%device)
    if not err:
      for line in lines:
        #print line
  ...
  try:
    ...
    if hw_platform and hw_platform == 'dell':
      ...
      if raid_to_unix_mapping and '/dev/%s'%disk_name in raid_to_unix_mapping.keys():
        ...
        raid = True
        controller_id = raid_to_unix_mapping['/dev/%s'%disk_name]['controller_number']
        device_id = raid_to_unix_mapping['/dev/%s'%disk_name]['device_id']
        disk_list, err = dell.get_hardware_raid_hard_drives(controller_id, device_id)
        if disk_list:
          for disk in disk_list:
            ...
            if disk_hw_info_all and disk['serial_number'] in disk_hw_info_all.keys():
              ...
              disk_dict['enclosure_id'] = disk_hw_info_all[disk['serial_number']]['enclosure_id']
