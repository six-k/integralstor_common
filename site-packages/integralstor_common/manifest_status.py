...
import json, os, shutil, datetime, sys, re, netifaces, socket, platform
import pprint
import integralstor_common
from integralstor_common import common, networking, command, hardware_utils, disks, zfs, services_management
...
def zfs_version():
  ...
  try:
    ...
    if lines and lines[0]:
      pos = lines[0].lower().find('loaded module')
      if pos != -1:
        zfs_ver = 'ZFS build ' + lines[0][pos+14:]
    else:
      zfs_ver = 'Could not determine..'
  except Exception, e:
      ...
      for line in lines:
        ...
        if match:
          ...
          if grps:
            ...
            if mem_total['unit'] == 'kB':
              mem_total['unit'] = 'GB'
              mem_total['value'] = float(mem_total['value'])/1024/1024
            elif mem_total['unit'] == 'mB':
              mem_total['unit'] = 'GB'
              mem_total['value'] = float(mem_total['value'])/1024
        ...
        if match:
          ...
          if grps:
            ...
            if mem_free['unit'] == 'kB':
              mem_free['unit'] = 'GB'
              mem_free['value'] = float(mem_free['value'])/1024/1024
            elif mem_free['unit'] == 'mB':
              mem_free['unit'] = 'GB'
              mem_free['value'] = float(mem_total['value'])/1024
  ...
  try:
    ...
    if platform == 'gridcell':
      ...
      for service_name  in services:
        stat, err = command.get_command_output('/sbin/service %s status'%service_name)
        ret = None
        rc = -1
        tup, err = command.execute_with_rc('/sbin/service %s status'%service_name)
        if tup:
          (ret, rc) = tup
        if err:
          raise Exception(err)
      ...
      for service_name, service_info in service_dict.items():
        if service_info['info']['status_str'] in ['Failed', 'Unknown state']:
          s[service_name] = [-1, service_info['info']['output_str']]
        else:
          s[service_name] = [0, service_info['info']['output_str']]
  except Exception, e:
      ...
      if hostname not in sd.keys():
        ...
        for disk_sn, disk in manifest["disks"].items():
          ...
          if disk_sn in sd[hostname]["disks"]:
            ...
            if dd['status'] and (not disk['hw_raid']) and (dd["status"] not in ['PASSED', 'OK']):
              node_status = 1
              temp_d["errors"].append("Disk with serial number %s is reporting SMART errors."%disk_sn)
            ...
            node_status = 1
            temp_d["errors"].append("Disk with serial number %s seems to be missing."%disk_sn)
          disks[disk_sn] = dd
        for td in sd[hostname]["disks"].keys():
          ...
          if td in manifest["disks"]:
            ...
            break
          if new_disk:
            temp_d["errors"].append("New disk detected. Disk with serial number %s  seems to be new."%td)
            node_status = 2
        #temp_d["disks"] = disks
        ...
        if sd[hostname]['services']:
          for service_name, service_info in sd[hostname]['services'].items():
            if service_info[0] != 0:
              temp_d['errors'].append('Service %s seems to have failed.'%service_name)
              node_status = 1
        ...
        for ifname, ifdict in manifest["interfaces"].items():
          ...
          if ifname in sd[hostname]["interfaces"]:
            id, err = _convert_to_status_interface_dict(sd[hostname]["interfaces"][ifname])
            if err:
              raise Exception(err)
          else:
            id["status"] = "Interface Missing"
            node_status = 1
            temp_d["errors"].append("Interface with number %s seems to be missing."%ifname)
        ...
        for ifname, id in temp_d['interfaces'].items():
          if ('ip_configured' in id or 'slave_to' in id)and id["status"] != 'up':
            node_status = 1
            temp_d["errors"].append("Interface %s is not up."%ifname)

        #print 'interfaces are ', temp_d['interfaces']

        if "memory" in sd[hostname]:
          if sd[hostname]["memory"]["mem_total"]["unit"] == "kB":
            sd[hostname]["memory"]["mem_total"]["value"] = str(int(sd[hostname]["memory"]["mem_total"]["value"])/1024)
            sd[hostname]["memory"]["mem_total"]["unit"] = "MB"
          if sd[hostname]["memory"]["mem_free"]["unit"] == "kB":
            sd[hostname]["memory"]["mem_free"]["value"] = str(int(sd[hostname]["memory"]["mem_free"]["value"])/1024)
        ...
        if temp_d["load_avg"]['15_min'] >= temp_d["load_avg"]['cpu_cores']:
          temp_d["errors"].append("The 15-minute load average (%.2f) has been high."%temp_d["load_avg"]['15_min'])
          node_status = "Degraded"
        if temp_d["load_avg"]['5_min'] >= temp_d["load_avg"]['cpu_cores']:
          temp_d["errors"].append("The 5-minute load average (%.2f) has been high."%temp_d["load_avg"]['5_min'])

        if 'ipmi_status' in temp_d:
          for status_item in temp_d['ipmi_status']:
            if status_item["status"] not in ['ok', 'nr']:
              temp_d["errors"].append('The %s of the %s is reporting errors'%(status_item["parameter_name"], status_item["component_name"]))
...
def main():
  ...
  pp.pprint(services_status())
  #ret, err = generate_manifest_info()
  #print ret.keys()
  #pp.pprint(interface_status())
  #pp.pprint(disk_info_and_status())
  #print zfs_version()
  #generate_status_info('/opt/integralstor/integralstor_unicell/config/status/master.manifest')
  #pp.pprint(generate_status_info('/opt/integralstor/integralstor_unicell/config/status/master.manifest'))
  #pp.pprint(generate_status_info('/opt/integralstor/integralstor_gridcell/config/status/master.manifest'))
  #r, err = generate_status_info('/opt/integralstor/integralstor_unicell/config/status/master.manifest')
  #if err:
  #  print err
  #print r.keys()
  #pp.pprint(r['unicell-f678.integralstor.lan']['disks'])
  #pp.pprint(services_status())
