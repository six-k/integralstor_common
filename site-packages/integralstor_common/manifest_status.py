...
def zfs_version():
    ...
    try:
        ...
        lines, err = command.get_command_output("modinfo zfs | grep -iw -e version | cut -d ':' -f 2", shell=True)
        if err:
                    ...
                    if disk_sn in sd[hostname]["disks"]:
                        ...
                        if dd['status']:
                            if  'hw_raid' in disk and (not disk['hw_raid']):
                                ...
                                node_status = 1
                                temp_d["errors"].append("Disk with serial number %s has errors."%disk_sn)
