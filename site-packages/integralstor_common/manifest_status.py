...
def zfs_version():
  zfs_ver = ""
  try:
    lines, err = command.get_command_output('dmesg | grep ZFS')
    if err:
...
def ipmi_status():
  ...
  try:
    ...
    for line in lines:
      ...
      if comp in["CPU Temp", "System Temp", "DIMMA1 Temp", "DIMMA2 Temp", "DIMMA3 Temp", "FAN1", "FAN2", "FAN3", "CPU1 Temperature", "MB1 Temperature", "CPU_FAN1"] and status != "ns":
        td = {}
      ...
      if hostname not in sd.keys():
        ...
        for disk_sn, disk in manifest["disks"].items():
          ...
          if disk_sn in sd[hostname]["disks"]:
            ...
            if dd['status'] and (dd["status"] not in ['PASSED', 'OK']):
              node_status = 1
        ...
        if sd[hostname]['services']:
          for service_name, service_info in sd[hostname]['services'].items():
            if service_info[0] != 0:
              temp_d['errors'].append('Service %s does not seem to be running.'%service_name)
              node_status = 1
