...
def get_psu_status():
  return_dict = {}
  try:
    lines, err = command.get_command_output('omreport chassis pwrsupplies -fmt xml')
    if err:
      raise Exception(err)
    if lines:
      #print lines
...
def get_hardware_raid_to_unix_mapping():
  mapping_dict = {}
  try:
    lines, err = command.get_command_output('omreport storage vdisk -fmt xml')
    if err:
      raise Exception(err)
    if lines:
      root = ElementTree.fromstring(''.join(lines))
...
def get_hardware_raid_hard_drives(controller_number, device_id):
  disk_list = []
  try:
    lines, err = command.get_command_output('omreport storage pdisk controller=%s vdisk=%d -fmt xml'%(controller_number, device_id))
    if err:
      raise Exception(err)
    if lines:
      root = ElementTree.fromstring(''.join(lines))
...
def get_alert_logs():
  ...
  try:
    alerts_dict = {}
    lines, err = command.get_command_output('omreport system alertlog -fmt xml')
    if err:
      raise Exception(err)
    if lines:
      root = ElementTree.fromstring(''.join(lines))
...
def get_idrac_addr():
  url = None
  try:
    lines, err = command.get_command_output('omreport system summary -fmt xml')
    if err:
      raise Exception(err)
    if lines:
      root = ElementTree.fromstring(''.join(lines))
...
def get_all_disks(controller_number):
  disks_dict = {}
  try:
    lines, err = command.get_command_output('omreport storage pdisk controller=%d -fmt xml'%controller_number)
    if err:
      raise Exception(err)
    if lines:
      root = ElementTree.fromstring(''.join(lines))
...
def blink_unblink_disk(action, controller_id, channel_id, enclosure_id, target_id):
  try:
    cmd = 'omconfig storage pdisk action=%s controller=%s pdisk=%s:%s:%s'%(action, controller_id, channel_id, enclosure_id, target_id)
    #print cmd
