...
def save_email_settings(d):
  ...
  try :
    ...
    if d1:
      ...
      ret, err = db.execute_iud(db_path, [["update email_config set server=?, port=?, username=?, pswd=?, tls=?, email_alerts=?, rcpt_list=? where id = ?", (d["server"], d["port"], d["username"], d["pswd"], d["tls"], d["email_alerts"], d["rcpt_list"], 1,)]])
      if err:
        raise Exception(err)
      ...
      ret, err = db.execute_iud(db_path, [["insert into email_config (server, port, username, pswd, tls, email_alerts, rcpt_list, id) values (?,?,?,?,?,?,?,?)", (d["server"], d["port"], d["username"], d["pswd"], d["tls"], d["email_alerts"], d["rcpt_list"],1, )]])
      if err:
        raise Exception(err)
