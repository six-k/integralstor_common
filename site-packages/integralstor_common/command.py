import os, subprocess, shlex, pwd
...
def execute(cmd = None, shell=False):
    ...
    try:
        ...
        proc = subprocess.Popen(comm_list, stdout = subprocess.PIPE, stderr = subprocess.PIPE, shell=shell)
        #proc.wait()
...
def execute_with_rc(cmd = None, shell=False, run_as_user_name=None):
    ...
    try:
        ...
        if shell:
            ...
            #comm_list = cmd.split()
            #Converting to str as the older python version of shlex in gridcell does not support unicode
        ...
        if run_as_user_name is not None:
            ...
            proc = subprocess.Popen(comm_list, preexec_fn=_demote(user_uid, user_gid), stdout = subprocess.PIPE, stderr = subprocess.PIPE, shell=shell)
        else:
            proc = subprocess.Popen(comm_list, stdout = subprocess.PIPE, stderr = subprocess.PIPE, shell=shell)
        #proc.wait()
        ret = proc.communicate()
...
def get_command_output(cmd, check_rc = True, shell=False):
    ...
    try:
        ret = None
        rc = -1
        tup, err = execute_with_rc(cmd,shell)
        if tup:
            (ret, rc) = tup
        if err:
            raise Exception(err)
