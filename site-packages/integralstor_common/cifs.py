...
"""
Change the currently set authentication method in the db.
"""
...
"""
Load the current authentication settings from the db
"""
def load_auth_settings():
  ...
  try:
    ...
    if err:
      raise Exception(err)
    d, err = db.read_single_row(db_path, "select * from samba_global_common where id=1")
    if err:
      raise Exception(err)
    if d and d["security"] == "ads":
      d1, err = db.read_single_row(db_path, "select * from samba_global_ad where id=1")
      if err:
        raise Exception(err)
      if d1:
        d.update(d1)
...
"""
Change the authentication settings in the db to what has been passed in the dict
"""
def save_auth_settings(d):
  ...
  try:
    ...
    cmd_list.append(cmd)
    if d["security"] == "ads":
      d1, err = db.read_single_row(db_path, "select * from samba_global_ad")
      if err:
        raise Exception(err)
      if d1:
...
"""
Remove all authentication settings from the db.
"""
...
"""
Load a share either based on name or by db id
"""
...
def create_share(name, comment, guest_ok, read_only, path, display_path, browseable, users, groups, vol):
  try:
    ...
    share_id, err = db.execute_iud(db_path, [["insert into samba_shares (name, vol, path, display_path, comment, read_only, guest_ok, browseable, share_id) values (?,?, ?,?,?,?,?,?,NULL)", (name, vol, path, display_path, comment, read_only, guest_ok, browseable,)]], True)
    if err:
      raise Exception(err)
    #print share_id, err
    cmd_list = []
...
"""
Load the list of users from the db who have access to a share.
"""
def load_valid_users_list(share_id):
  ...
  try:
    ...
    if err:
      raise Exception(err)
    l, err = db.read_multiple_rows(db_path, 'select * from samba_valid_users where share_id = %s'%share_id)
    if err:
      raise Exception(err)
  except Exception, e:
...
"""
Generate the share body in smb.conf. Only generates the parts that are common to unicell and gridcell
"""
def generate_common_share_body(f, workgroup, path, read_only, browseable, guest_ok, user_list, group_list, comment, auth_method, extra_share_param_lines = None):
  try:
    if not f:
      raise Exception('No file handle passed')
    if comment:
    ...
    #Move this to the gridcell specific code
...
"""
Generates the common global header . unicell/gridcell may add parts that are unique to them.
"""
...
"""
Generates the common global section. unicell/gridcell may add parts that are unique to them.
"""
