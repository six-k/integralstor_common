...
def generate_nginx_conf(ssl=False, ssl_cert_file = None, ssl_key_file = None):
  try:
    ...
    with open('/etc/nginx/sites-enabled/integral_view_nginx.conf', 'w') as f:
      ...
      f.write(' server unix:////opt/integralstor/%s/integral_view/integral_view.sock;\n'%platform)
      f.write('}\n')
      f.write('\n')
      f.write('server {\n')
      if ssl:
...
def main():
  ...
  #print generate_nginx_conf(True, '/opt/integralstor/pki/blah/blah.cert', '/opt/integralstor/pki/blah/blah.cert')
  print get_nginx_access_mode()
