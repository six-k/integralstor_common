...
def get_new_ug_list(ace_list, all_ug_list, type):
  ...
  try:
    if not ace_list:
      raise Exception('No ACL list passed')
    if not all_ug_list:
      raise Exception('No user/group list passed')
    if type not in ['user', 'group']:
      raise Exception('Invalid type passed')
...
def set_ace_entries(path, d):
  ...
  try:
    ...
    for name, perms_list in users.items():
      ...
      if err:
        raise Exception(err)
      cmd += 'user:%s:%s,'%(name, perms_str)
      cmd += 'default:user:%s:%s,'%(name, perms_str)

    for name, perms_list in groups.items():
      perms_str, err = _convert_to_ace_str(perms_list[0], perms_list[1], perms_list[2])
      if err:
        raise Exception(err)
      cmd += 'group:%s:%s,'%(name, perms_str)
      cmd += 'default:group:%s:%s,'%(name, perms_str)
