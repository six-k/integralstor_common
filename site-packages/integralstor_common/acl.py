...
'''
def get_ug_list_from_aces(ace_list, type):
  #Type shd be either 'user' or 'group'
  ug_list = []
  try:
    if not ace_list:
    ...
    if type not in ['user', 'group']:
      raise Exception('Invalid type passed')
    for ace in ace_list:
      if ace[0] == 'default' and ace[1] == type and ace[2]:
        ug_list.append(ace[2])
  except Exception, e:
    return None, 'Error retrieving user/group list from ACLs : %s'%str(e)
  else:
    return ug_list, None

    #existing_ugs, err = get_ug_list_from_aces(ace_list, type)
'''
...
def delete_ace(path, name, type, recursive = False):
    try:
        ...
        if type not in ['user', 'group']:
            raise Exception('Invalid type passed')
        if recursive:
            #cmd = 'setfacl -R -x %s:%s,default:%s:%s "%s"'%(type, name.replace(' ', '\\ '), type, name.replace(' ', '\\ '), path)
            cmd = 'setfacl -R -x %s:"%s",default:%s:"%s" "%s"'%(type, name, type, name, path)
        else:
            #cmd = 'setfacl -x %s:%s,default:%s:%s "%s"'%(type, name.replace(' ', '\\ '), type, name.replace(' ', '\\ '), path)
            cmd = 'setfacl  -x %s:"%s",default:%s:"%s" "%s"'%(type, name, type, name, path)
        #print cmd
...
def add_ace_entries(path, users, groups, recursive):
    try:
        if recursive:
            cmd = r'setfacl -R -m'
        else:
            cmd = r'setfacl -m '
        for user in users:
            #cmd += 'u:%s:---,d:u:%s:---,'%(user.replace(' ', '\ '), user.replace(' ', '\ '))
            cmd += 'u:"%s":---,d:u:"%s":---,'%(user, user)
        for group in groups:
            #cmd += 'g:%s:---,d:g:%s:---,'%(group.replace(' ', '\ '), group.replace(' ', '\ '))
...
def set_ace_entries(path, d):
    ...
    try:
        '''
        lines, err = command.get_command_output('getfacl -R %s'%path)
        if err:
          raise Exception('Error backing up current ACL : %s'%err)
        with open('/tmp/acl_backup_%s'%
        '''
        ...
        for name, perms_list in users.items():
            perms_str, err = _convert_to_ace_str(perms_list[0], perms_list[1], perms_list[2])
            if err:
                raise Exception(err)
        ...
        for name, perms_list in groups.items():
            ...
            cmd += 'default:group:%s:%s,'%(name.replace(' ', '\ '), perms_str)
