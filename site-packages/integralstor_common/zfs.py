...
def set_default_pool_quota(pool_name):
    try:
        ...
        if quota_str:
            ret, err = set_property(pool_name, 'quota', quota_str)
...
def create_pool(pool_name, type, data_vdev_list, log_vdev = None, dedup = False):
    try:
        ...
        if type == 'mirror':
            ...
            cmd = '/sbin/zpool create -o autoexpand=on  -f  %s raidz1 '%(pool_name)
            ...
            cmd = '/sbin/zpool create -o autoexpand=on  -f  %s raidz2 '%(pool_name)
            for vdev in data_vdev_list:
                if vdev not in free_disk_ids:
                    raise Exception('Specified disk already in use in another pool!')
                cmd += ' %s '%vdev
        ...
        ret, err = set_default_pool_quota(pool_name)
        if err:
            raise Exception(err)
