...
def schedule_remote_replication(description, dataset,destination_ip,destination_user_name,destination_pool):
  try:
    ...
    if err:
      raise Exception(err)
    source_pool,source_dataset = dataset.split("/")
    ...
    ret,err = scheduler_utils.add_task(description,[{'Replication':cmd}], task_type_id = 4, run_as_user_name='replicator', retry_interval=60)
    print ret, err
    if err:
      raise Exception(err)
    #print "\n--- ", ret
