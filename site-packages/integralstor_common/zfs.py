...
def can_expand_pool(pool_name):
    # Check if the pool can be expanded. If it can, return the new pool type
    # also.
    try:
        if not pool_name:
...
def get_quota(path, ug_name, user=True):
    # Returns the quota for the particular user or group. Set user=True if
    # user else it is a group
    quota = None
    try:
        if not ug_name:
...
def set_quota(path, ug_name, quota_amt, user=True):
    # Set the quota for the particular user or group. Set user=True if user
    # else it is a group. QUota_amt shd include the units
    try:
        if not ug_name:
            raise Exception('No user or group name passed')
        if not path:
