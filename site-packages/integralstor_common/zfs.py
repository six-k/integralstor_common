...
from integralstor_common import command, disks, ramdisk,common,scheduler_utils,db
...
def get_latest_pool_snapshot(name=None):
  cmd = None
  try:
    if name:
      cmd = "/sbin/zfs list -r -t snapshot %s -o name,creation -s creation | grep %s@zfs | tail -1"%(name,name)
    else:
      cmd = "/sbin/zfs list -t snapshot -o name,creation -s creation | grep %s@ | tail -1"%(name)
    line, err = command.get_command_output(cmd)
    if err:
      raise Exception(err)
    else:
...
def get_latest_snapshot(name=None):
  cmd = None
  try:
    if name:
      cmd = "/sbin/zfs list -r -t snapshot %s -o name,creation -s creation | tail -1"%(name)
    else:
...
def get_property(target, prop_name):
  ...
  try:
    ...
    if lines and len(lines) == 2:
      ...
      d, err = _get_property_compononents(lines[1], property_index, value_index, source_index)
      if err:
        raise Exception(err)
  except Exception, e:
    if prop_name:
  ...
  try:
    ...
    if free_disks:
      ...
      if pool_type == 'mirror':
        ...
        if pool_type == 'raid10':
          #Temporarily fake the num_raid_disks for a raid10 pool to maintain minimum logic below
          num_raid_disks = 2
        if num_free_disks < (int(stripe_width)*int(num_raid_disks)):
          raise Exception ('Insufficient disks to form the specified pool type')
        l = []
      ...
      raise Exception('There are no free disks of the specified type to form the pool')
  except Exception, e:
...
def can_expand_pool(pool_name):
  ...
  try:
    ...
    if type in ['raid1', 'raid10']:
      ...
      if len(free_disks) < 2:
        raise Exception('Insufficient free disks to expand the pool to a RAID-10')
      new_pool_type = 'RAID-10'
    elif type in ['raid5', 'raid6', 'raid50', 'raid60']:
      #Can expand to a RAID-50
      num_raid_disks = len(pool['config']['pool']['root']['children'][0]['children'])
      if len(free_disks) < num_raid_disks:
        raise Exception('Insufficient free disks to expand the pool. Need %d disk(s). Have %d disk(s) available.'%(num_raid_disks, len(free_disks)))
  ...
  try:
    ...
    if type in ['raid1', 'raid10']:
      ...
      if len(free_disk_ids) < 2:
        raise Exception('Insufficient free disks to expand the pool to a RAID-10')
      cmd = '/sbin/zpool add -f  %s mirror %s %s'%(pool_name, free_disk_ids[0], free_disk_ids[1])
    elif type in ['raid5', 'raid6', 'raid50', 'raid60']:
      #Can expand to a RAID-50
      num_raid_disks = len(pool['config']['pool']['root']['children'][0]['children'])
      if len(free_disk_ids) < num_raid_disks:
        raise Exception('Insufficient free disks to expand the pool. Need %d disk(s). Have %d disk(s) available.'%(num_raid_disks, len(free_disk_ids)))
      if type in ['raid5', 'raid50']:
    ...
    if not cmd:
      raise Exception('Could not generate the appropriate expansion parameters')
    #print cmd
    lines, err = command.get_command_output(cmd)
    if err:
      raise Exception(err)
...
def set_pool_log_vdev(pool_name, vdev):
  try:
    ...
    return False, 'Error setting  pool log vdev : %s '%str(e)
  else:
    return True, None

def set_pool_cache_vdev(pool_name, vdev):
  try:
    if (not pool_name) or (not vdev):
      raise Exception('Pool name or vdev not specified')
    ...
    return False, 'Error setting  pool cache vdev : %s '%str(e)
  else:
    return True, None
...
def get_pool_usage(pool_name):
  ...
  try:
    ...
    if lines:
      ...
      td, err = _get_numbers_and_units(components[1])
      if err:
        raise Exception(err)
      d['size'] = td
      td, err = _get_numbers_and_units(components[2])
      if err:
        raise Exception(err)
      d['used'] = td
      td, err = _get_numbers_and_units(components[3])
      if err:
        raise Exception(err)
      d['free'] = td
      td, err = _get_numbers_and_units(components[4])
      if err:
        raise Exception(err)
      d['used_percent'] = td
  except Exception, e:
  ...
  try:
    ...
    for pool in pools:
      ...
      if not spares_are_free_disks:
        ...
        #Check if the spares are available before including them as free disks
        if 'spares' in pool['config'] and pool['config']['spares']:
      ...
      for sn, disk in all_disks.items():
        if disk_type:
          if disk_type == 'rotational' and not disk['rotational']:
            #Asking for rotational but current disk is flash so continue
            continue
          if disk_type == 'flash' and disk['rotational']:
            #Asking for flash but current disk is rotational so continue
            continue
...
def get_free_disks_for_spares(pool_name):
  ...
  try:
    ...
    if err:
      raise Exception(err)
    #Now make sure that we dont return disks that are already a spare in the requested pool
    pool_spares = None
    if 'spares' in pool['config'] and pool['config']['spares']:
    ...
    for disk in fd:
      if not pool_spares or disk['id'] not in pool_spares:
        #Not already a spare in this pool but it is ok if it is a spare in some other pool
        free_disks.append(disk)
  except Exception, e:
...
def add_spares_to_pool(pool_name, num_spares=1):
  try:
    ...
    if len(free_disks) < num_spares:
      raise Exception('Insufficient number of free disks available for spares. %d free disks currently available.'%len(free_disks))
    lines, err = command.get_command_output('zpool set autoreplace=on %s'%pool_name)
    if err:
      raise Exception(err)
...
def remove_a_spare_from_pool(pool_name):
  try:
...
def set_quota(path, ug_name, quota_amt, user=True):
  #Set the quota for the particular user or group. Set user=True if user else it is a group. QUota_amt shd include the units
  try:
    if not ug_name:
      raise Exception('No user or group name passed')
    if not path:
    ...
    return False, "Error setting quota : %s"%str(e)
  else:
    return True, None
"""
...
"""
def remote_replication_command(pool,host,user,receive_pool):
  try:
    db_path,err = common.get_db_path()
    pool,dataset = pool.split("/")
    path,err = common.get_shell_scripts_path()
    if not err:
      path = path + "/replicator.sh"
    cmd = "/bin/bash %s %s %s %s %s %s"%(path,pool,dataset,receive_pool,user,host) 
    print cmd
    ...
    latest,err = get_latest_pool_snapshot(pool)
    if err:
      raise Exception("No Snapshots creted. Please create or scheduler a snapshot")
    command = "select * from scheduler_tasks where task_name LIKE 'Replication%' ORDER BY initiate_time  DESC LIMIT 1"
    prev,err_com = db.read_single_row(db_path,command)
    if not prev:
      cmd = 'zfs send %s | ssh %s@%s "zfs receive -Fdv %s"'%(latest,user,host,receive_pool)
    else:
      prev_snap = prev['task_name'].split(' ')[-1]
      cmd = 'zfs send -I %s %s | ssh %s@%s "zfs receive -Fdv %s"'%(prev_snap,latest,user,host,receive_pool)
    print cmd    
    schedule,err = scheduler_utils.schedule_a_job(db_path,"Replication for %s"%(latest),[{'replication':cmd}],extra = {'deletable':0})
    print schedule,err
    return latest,None
    """
  except Exception,e:
...
def execute_remote_replication():
  tasks = "select * from pool_repl"
  try:
    db_path,err = common.get_db_path()
    pools,err = db.read_multiple_rows(db_path,tasks)
    if not pools:
      return None,"No Pools found for replication"
    else:
      for pool in pools:
        status,err = remote_replication_command(pool["pool"],pool["ip"],pool["user"],pool["dest"])
      return pools,False
  except Exception,e:
    return None,e
...
def main():
  ...
  #d,err = remote_replication_command('rr','192.168.1.208','root','rr')
  #d, err = get_all_components_status()
  #d, err = get_all_zvols()
  #d, err = get_pool('p1')
  #d, err = get_quota('p1', 'ram', False)
  #d, err = set_quota('p1', 'ram', 'none')
  #print d, err
  #d, err = get_all_quotas('p1')
  #print d, err
  #d, err = set_quota('p1', 'ram', '1GB')
  #print d, err
  #d, err = get_all_quotas('p1')
  #print d, err
  #d, err = set_quota('p1', 'ram', '2MB')
  #print d, err
  #d, err = get_all_quotas('p1')
  #print d, err
  #d, err = add_spares_to_pool('p1', 1)
  #d, err = get_pool_spares('p1')
  #d, err = remove_spares_from_pool('p1')
