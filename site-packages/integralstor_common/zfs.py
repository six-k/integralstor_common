...
def remote_replication_command(pool,host,user,receive_pool):
  try:
    ...
    if not prev:
      cmd = 'zfs send -R %s | ssh %s@%s "zfs receive -Fduv %s"'%(latest,user,host,receive_pool)
    else:
      print prev
      prev_snap = prev['task_name'].split(' ')[-1]
      cmd = 'zfs send -R -i %s %s | ssh %s@%s "zfs receive -Fduv %s"'%(prev_snap,latest,user,host,receive_pool)
    print cmd    
