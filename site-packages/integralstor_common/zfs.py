...
def get_all_snapshot_schedules():
  ...
  try:
    ...
    if auto_snapshots_exist:
      lines, err = command.get_command_output('zfs get all | grep auto-snapshot', shell=True)
      if err:
        raise Exception(err)
      if lines:
        for line in lines:
...
def create_pool(pool_name, type, data_vdev_list, log_vdev = None, dedup = False):
  try:
    ...
    if type == 'mirror':
      ...
      cmd = '/sbin/zpool create -o autoexpand=on -f  %s mirror %s %s'%(pool_name, data_vdev_list[0], data_vdev_list[1])
    elif type == 'raid5':
    ...
    cmd = 'zfs set  acltype=posixacl %s'%pool_name
    lines, err = command.get_command_output(cmd)
    if err:
...
def get_pool_usage(pool_name):
  ...
  try:
    ...
    if 'fragmentation' in pool_props:
      td, err =  _get_size_components(pool_props['fragmentation']['value'])
      if err:
        raise Exception(err)
      d['fragmentation'] = td
...
"""
def schedule_remote_replication(dataset,host,user,receive_pool):
  try:
    db_path,err = common.get_db_path()
    pool,dataset = dataset.split("/")
    ...
    #print cmd
    schedule,err = scheduler_utils.schedule_a_job(db_path,"Replication for %s-%s"%(pool,dataset),[{'replication':cmd}],retries=-2,extra = {'deletable':0}, user_name='replicator')
    if err:
      raise Exception(err)
    return True,None
  except Exception,e:
...
def execute_remote_replication():
  tasks = "select * from dataset_repl"
  try:
    db_path,err = common.get_db_path()
    if err:
      raise Exception(err)
    datasets,err = db.read_multiple_rows(db_path,tasks)
    if err:
      raise Exception(err)
    if not datasets:
      ...
      for dataset in datasets:
        status,err = schedule_remote_replication(dataset["dataset"],dataset["ip"],dataset["user"],dataset["dest"])
