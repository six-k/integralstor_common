...
def get_ftp_config():
  ...
  try:
    with open('/etc/vsftpd/vsftpd.conf', 'r') as f:
      for line in f:
        res = re.search('[\s]*(?i)rsa_cert_file[\s]*=[\s]*([\S]+)', line)
        if res and res.groups():
          ...
          if cert_file_end_pos:
            ...
            if cert_file_start_pos:
              config['cert_name'] = cert_file_loc[cert_file_start_pos+1:cert_file_end_pos]
...
def create_ftp_user_dirs(dataset, users):
  #This is needed to make sure that every local user has their FTP directory created
  try:
    #print users
    for user in users:
...
def update_ftp_config(config):
  try:
    ...
    with open('/tmp/vsftpd.conf', 'w') as f:
      f.write("# AutoGenerated by IntegralStor. Do not change this file manually \n")
