        ...
        if d:
            if d["email_audit"]:
                ret, err = mail.send_mail(d["server"], d["port"], d["username"], d["pswd"], d["tls"],
                                          d["rcpt_list"], "Audit from Integralstor " + networking.get_hostname()[0], audit_description)
                if err:
                    raise Exception(err)
...
def _get_audit_file_path():
    # Return the audit file path. Create the audit directory and file if it
    # does not exist
...
def get_log_file_list():
    # Get a list of dicts with each dict having a date and all the rotated log
    # files for that date
