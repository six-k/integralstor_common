...
def audit(audit_action, audit_str, ip_meta):
  ...
  try:
    ...
    if d:
      if d["email_audit"]:
        ret, err = mail.send_mail(d["server"], d["port"], d["username"], d["pswd"], d["tls"], d["rcpt_list"], "Audit from Integralstor "+networking.get_hostname()[0], audit_email)
        if err:
          raise Exception(err)
        if ret:
          with open(filename, "a") as f:
            f.write("\n%-13d %s\n"%(t, "Error sending email audit %s"%ret))
          f.close()
...
def _parse_audit_line(str):
  try:
    ...
    "remove_background_task": "Remove background task ",
    "create_remote_replication": "Create remote replication ",
    "modify_remote_replication": "Modify remote replication ",
    "remove_remote_replication": "Remove remote replication ",
    "remove_ssh_user_key": "Remove ssh user key ",
    "upload_ssh_user_key": "Upload ssh user key ",
    "remove_ssh_host_key": "Remove ssh host key ",
    "upload_ssh_host_key": "Upload ssh host key ",
    }
    ...
    m = re.match("([\d]+)[\s]+([0-9]+\.[0-9]+\.[0-9]+\.[0-9]+)[\s]+([a-zA-Z0-9_]+)[\s]+(.*)", str)
    d = None
    if m:
      t = m.groups()
      if len(t) == 4:
