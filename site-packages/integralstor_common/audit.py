...
def audit(audit_action, audit_str, ip_meta, system_initiated = False):
    ...
    try:
        ...
        if d:
            if d["email_audit"]:
                ret, err = mail.send_mail(d["server"], d["port"], d["username"], d["pswd"], d["tls"], d["rcpt_list"], "Audit from Integralstor "+networking.get_hostname()[0], audit_description)
                if err:
                    raise Exception(err)
...
def _parse_audit_line(str):
    ...
    '''
        d = {}
        d["time"] = time.strftime("%a, %d %b %Y %H:%M:%S", time.localtime(int(str[:13])))
        d["ip"] = str[14:30]
        action = str[31:56].strip ()
        ...
        return None, 'Error parsing audit log line : %s'%(e)
    else:
        return d, None
...
if __name__ == "__main__":
    ...
    #print _parse_audit_line('1485239641    system-initiated task_start                Replication of zpool/Unit1 to pool zpool on machine 192.168.1.247')
    print _parse_audit_line('1485243283    192.168.1.79     delete_zfs_snapshot       Deleted ZFS snapshot zpool/Unit1@test-dnap-2')

#print audit("remote_replication_status_start", "ldkfladkjldakf", None, system_initiated = True)
