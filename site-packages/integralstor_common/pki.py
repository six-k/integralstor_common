...
def generate_ssh_key(user="root"):
  try:
    ...
    if err:
      ...
      os.mkdir('/%s/.ssh/'%user)
      shutil.copyfile('%s.pub'%user,_get_key_file(user))      
      return True,None
  except Exception,e:
...
def get_ssh_key(user='root'):
  ...
  try:  
    ...
    return False,"Unable to find the ssh key file : %s"%str(e)
...
def get_ssh_key():
  try:
    shutil.copyfile("/root/.ssh/id_rsa.pub","/opt/integralstor/integralstor_unicell/integral_view/static/id_rsa.pub")
