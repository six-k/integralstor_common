...
def generate_ssh_key(user='replicator'):
  try:
    ...
    runuser = "runuser -l %s -c "'"%s"'" "%(user,cmd)
    print cmd,runuser
    ...
    if err:
      ...
      shutil.copyfile('%s.pub'%user,_get_public_key_file(user))      
      return True,None
  except Exception,e:
...
def ssh_dir_permissions(user='replicator'):
  try:
    ssh_perm = "runuser -l %s -c 'chmod 700 %s'"%(user,_get_ssh_dir(user))
    (ret, rc), err = command.execute_with_rc(ssh_perm,True)
    authorized_key = "runuser -l %s -c 'chmod 640 %s/authorized_keys'"%(user,_get_ssh_dir(user))
...
def get_ssh_key(user='replicator'):
  try:
    ...
    shutil.copyfile(_get_public_key_file(user),"/opt/integralstor/integralstor_unicell/integral_view/static/id_rsa.pub")
...
def get_ssh_host_identity_key():
  try:
    shutil.copyfile("/etc/ssh/ssh_host_rsa_key.pub","/opt/integralstor/integralstor_unicell/integral_view/static/ssh_host_rsa_key.pub")
