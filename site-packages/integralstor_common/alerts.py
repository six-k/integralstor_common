...
def raise_alert(alert_url, msg):
    try:
        ...
        if err:
            raise Exception(err)
        url = "http://%s/%s?msg=%s" % (url_host,
                                       url_component, urllib.quote_plus(msg))
        # print url
...
def load_alerts(fname=None, last_n=0):
    # Read the alerts file. If the last line does not havethe dash pattern
...
def _get_alerts_file_path():
    # Return the alerts file path. Create the alerts directory and file if it
    # does not exist
