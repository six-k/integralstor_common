import time, os, fcntl, os.path, re

from integralstor_common import file_processing, common, logs, mail

"""
Raise an alert for each message in msg_list - send an email if the system has been configured to.
"""
def raise_alert(msg_list):
  try:
    ...
    if d:
      if d["email_alerts"]:
        ret, err = mail.send_mail(d["server"], d["port"], d["username"], d["pswd"], d["tls"], d["rcpt_list"], "Alert from Fractal-view", '\n'.join(msg_list))
        if err:
          raise Exception(err)
        if ret:
          with open(filename, "a") as f:
...
"""
Load the alerts from the specified fname or the default alerts log file.
"""
...
"""
Get a list of dicts with each dict having a date and all the rotated log files names for that date
"""
def get_log_file_list():
  ...
  try:
    ...
    if err:
      raise Exception(err)
    #Get all file names in alerts_dir except alerts.log
    l, err = logs.get_log_file_list(alerts_dir, "alerts.log")
    if err:
      raise Exception(err)
    #Generate a display ready list
...
"""
To be called to generate an alert through a URL. Currently not used so commenting out for now.
