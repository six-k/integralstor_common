...
def get_remote_replications_with (source_dataset, destination_ip, destination_pool):
  ...
  try:
    ...
    if err:
      raise Exception (err)
    cmd = "select * from remote_replications where source_dataset='%s' and destination_ip='%s' and destination_pool='%s'"%(source_dataset, destination_ip, destination_pool)
    replications, err = db.read_multiple_rows (db_path, cmd)
    if err:
      raise Exception (err)
    ...
    if replications is not None:
      #print 'replications - ', replications
      for replication in replications:
        repls, err = get_remote_replications (replication['remote_replication_id'])
        if err:
          raise Exception (err)
...
def schedule_remote_replication(description, dataset,destination_ip,destination_user_name,destination_pool):
  try:
    ...
    rr, err = get_remote_replications_with (dataset, destination_ip, destination_pool)
    print rr, "\n"
    if err:
      raise Exception (err)
    if not rr:
      raise Exception ('Could not locate the specified remote replication task')
    
    # Check for active replication tasks
    # Modified remote replication is assigned a new cron_task_id. 
    # TOFIX: Another replication task might still be 'running' with the old cron_task_id; old cron_task_id may not be in the records, so check for matching descriptions to assert
    existing_tasks, err = scheduler_utils.get_tasks_by_cron_task_id (rr[0]['cron_task_id'])
    if err is not None:
      raise Exception (err)
    if existing_tasks:
      for task in existing_tasks:
	if str (task['status']) == "running" and str (task['task_type_id']) == "4":
	  raise Exception ("Will not commence the replication; a background task with the same description is yet to complete its replication!")
...
if __name__ == "__main__":
  ...
  #print schedule_remote_replication("Replication of try-try/send to pool zpool on machine 192.168.1.212","test-pool/test-send", "192.168.1.129", "replicator", "p1")
  print ""
