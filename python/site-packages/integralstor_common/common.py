...
def get_platform_root():
  ...
  try:
    ...
    root = '/opt/integralstor/integralstor-%s'%platform
  except Exception, e:
...
def get_ramdisk_config_file_location():
  ...
  try:
    ...
    if err:
      raise Exception(err)
    if not platform:
      raise Exception('Could not retrieve platform')
    ret =  "/opt/integralstor/integralstor-%s"%platform
  except Exception, e:
    return None, 'Error getting ramdisk config file location : %s'%str(e)
  else:
    return ret, None
...
def get_devel_files_path():
  return "/opt/fractalio/devel/files", None
...
def get_audit_url_host():
  return "integralstor.fractalio.lan", None
