...
def remove_bond(bond_name):
  ...
  try:
    ...
    (ret, rc), err = command.execute_with_rc('ifconfig %s down'%bond_name)
    if err:
      raise Exception(err)
    if rc != 0:
      err = ''
...
def set_hostname(hostname, domain_name):
  try:
    ...
    with open('/etc/sysconfig/network', 'w') as f:
      ...
      f.flush()
    f.close()
    (r, rc), err = command.execute_with_rc('hostname %s'%hostname)
    if err:
      raise Exception(err)
    if rc == 0:
...
def restart_networking():
  ...
  try:
    (ret, rc), err = command.execute_with_rc('service network restart')
    if err:
      raise Exception(err)
    if rc != 0:
      err = ''
