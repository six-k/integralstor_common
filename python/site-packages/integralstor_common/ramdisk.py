...
def get_ramdisks_config():
  ...
  try:
    lines = None
    with open('%s/ramdisks'%common.get_ramdisk_config_file_location(), 'r') as f:
      lines = f.readlines()
    if lines:
      for line in lines:
        components = line.split()
...
def insert_into_ramdisks_config(pool, path, size):
  try:
    with open('%s/ramdisks'%common.get_ramdisk_config_file_location(), 'a') as f:
      f.write('%s %s %s\n'%(pool, path, size))
      f.flush()
  except Exception, e:
...
def remove_from_ramdisks_config(pool, path):
  try:
    ramdisks, err = get_ramdisks_config()
    if ramdisks:
      new_ramdisks = []
      ...
      with open('/tmp/ramdisks', 'w') as f:
        for r in new_ramdisks:
          f.write('%s %s\n'%(r['path'], r['size']))
          f.flush()
      shutil.move('/tmp/ramdisks', '%s/ramdisks'%common.get_ramdisk_config_file_location())
    else: 
...
def main():
  ...
  print destroy_ramdisk('ramdisk1', 'pool1')
