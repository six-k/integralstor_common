...
def get_ramdisks_config():
  ...
  try:
    lines = None
    root, err = common.get_platform_root()
    if err:
      raise Exception(err)
    if not root:
      raise Exception('Could not determine platform root')
    with open('%s/ramdisks'%root, 'r') as f:
      lines = f.readlines()
    if lines:
      for line in lines:
        components = line.split()
...
def insert_into_ramdisks_config(pool, path, size):
  try:
    ...
    if err:
      raise Exception(err)
    with open('%s/ramdisks'%root, 'a') as f:
      f.write('%s %s %s\n'%(pool, path, size))
      f.flush()
  except Exception, e:
    ...
    if ramdisks:
      ...
      with open('/tmp/ramdisks', 'w') as f:
        for r in new_ramdisks:
          ...
          f.flush()
      shutil.move('/tmp/ramdisks', '%s/ramdisks'%root)
    else: 
