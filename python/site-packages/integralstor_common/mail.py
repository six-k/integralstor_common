...
def load_email_settings():
  ...
  try :
    ...
    if err:
      raise Exception(err)
    d, err = db.read_single_row("%s/integral_view_config.db"%db_path, "select * from email_config where id = 1")
    print '3'
    if err:
      raise Exception(err)
  except Exception, e:
...
def save_email_settings(d):
  ...
  try :
    ...
    d1 = cur.fetchone()
    '''
    d1, err = db.read_single_row("%s/integral_view_config.db"%db_path, "select * from email_config")
    if err:
      raise Exception(err)
    if d1:
      ret, err = db.execute_iud("%s/integral_view_config.db"%db_path, ["update email_config set server=?, port=?, username=?, pswd=?, tls=?, email_alerts=?, rcpt_list=? where id = ?", (d["server"], d["port"], d["username"], d["pswd"], d["tls"], d["email_alerts"], d["rcpt_list"], 1,)])
      if err:
        raise Exception(err)
      ...
      ret, err = db.execute_iud("%s/integral_view_config.db"%db_path, ["insert into email_config (server, port, username, pswd, tls, email_alerts, rcpt_list, id) values (?,?,?,?,?,?,?,?)", (d["server"], d["port"], d["username"], d["pswd"], d["tls"], d["email_alerts"], d["rcpt_list"],1, )])
      if err:
        raise Exception(err)
