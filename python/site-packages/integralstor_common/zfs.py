...
def get_vdev_type(d):
  ...
  try:
    ...
    '''
    if '_root' not in d:
      raise Exception('Unknown root component.')
    if 'components' not in d:
      raise Exception('No components in vdev.')
    root = d['_root']
    if root not in d['components']:
      raise Exception('No root component in vdev.')
    if not d['components'][root]:
...
def create_zvol(pool, name, properties, size, unit):
  try:
    ...
    cmd = 'zfs create  -V %d%s '%(size,unit)
    if properties:
      for pname, pvalue in properties.items():
        cmd += '-o %s=%s '%(pname, pvalue)
    cmd += ' %s/%s'%(pool, name)
