...
def _get_property_compononents(line):
  d = None
  try:
    ...
    if ds_components:
      if ds_components[0].strip():
        d = {}
        d['name'] = ds_components[1]
        d['value'] = ds_components[2]
        d['source'] = ds_components[3]
  except Exception, e:
    return None, str(e)
  else:
    return d, None
...
def get_dataset_properties(ds_name):
  ...
  try:
    ...
    if lines:
      for line in lines:
        td, err = _get_property_compononents(line)
        if td:
