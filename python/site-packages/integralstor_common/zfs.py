...
def get_snapshots(name=None):
  ...
  try:
    ...
    for snap in snapshots:
      prop, err = get_dataset_properties(snap['name'])
      if prop:
      ...
      for line in lines:
        ...
        if td:
          if td['name'] != pool_name:
            prop, err = get_dataset_properties(td['name'])
            if not prop:
              if err:
                raise Exception(err)
              else:
...
def get_dataset_properties(ds_name):
  properties = {}
  try:
    cmd = 'zfs get all %s '%ds_name
    ret, rc = command.execute_with_rc(cmd)
    if rc == 0:
      lines = command.get_output_list(ret)
    else:
    ...
    return None, 'Error retrieving dataset properties : %s'%str(e)
  else:
    return properties, None

def set_dataset_property(ds_name, prop_name, prop_value):
  try:
    if (not ds_name) or (not prop_name) or (not prop_value):
      raise Exception('Required parameters not passed')

    cmd = 'zfs set %s=%s %s'%(prop_name, prop_value, ds_name)
    print cmd
    ret, rc = command.execute_with_rc(cmd)
    print rc
    if rc == 0:
    ...
    if prop_name:
      return False, 'Error saving dataset property %s : %s '%(prop_name, str(e))
    else:
      return False, 'Error saving dataset property  : %s '%str(e)
  else:
    return True, None
...
def create_pool(pool_name, type, data_vdev_list, log_vdev = None):
  try:
    ...
    if type == 'mirror':
      ...
      if (data_vdev_list[0] not in free_disks) or (data_vdev_list[1] not in free_disks):
...
def main():
  #d, err = get_pools()
  #d, err = get_snapshots()
  #d, err = get_dataset_properties('frzpool/normal')
  #d, err = get_datasets_in_pool('frzpool')
