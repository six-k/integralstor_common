...
def create_pool_data_vdev_list(pool_type, stripe_width = None):
  ...
  try:
    ...
    if free_disks:
      ...
      if pool_type == 'mirror':
        ...
        if num_free_disks < (int(stripe_width)*3):
          raise Exception ('Insufficient disks to form a RAID10 pool')
        d = None
        for i in range(stripe_width*3):
          if (i+1)%3 == 0:
            if d:
              vdev_list.append(d)
            d = {}
...
def main():
  #d, err = get_pools()
  #d, err = get_snapshots()
  #d, err = get_properties('frzpool/normal')
